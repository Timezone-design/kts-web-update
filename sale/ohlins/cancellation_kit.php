<?php
	...
	/* 各種項目設定 */
		...
		/* ファイル名 */
		$file_name = "cancellation_kit.php";

		/* 商品名 */
		$set_name = "OHLINS キャンセレーションキット";

		/* DBパス */
		$set_db = "db/cancellation_kit.csv";
...
<?php
	...
	print "				<td>"; print_r($db_data[0][2]); print "（税抜）</td>\n";
	print "			</tr>\n";
	/* 適合データの処理 */
	for($cnt = 1; $cnt < $i; $cnt++){
		...
		if($db_data[$cnt][0] == $maker){
			...
			print_r($db_data[$cnt][4]);
			print ">\n";
			print "				<td>"; print_r($db_data[$cnt][1]); print "</td>\n";
			/* 販売価格に3桁毎のカンマを追加処理 */
			$price1 = number_format($db_data[$cnt][2]);
			$price2 = number_format($db_data[$cnt][3]);
