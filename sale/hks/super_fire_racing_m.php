<?php
	...
	/* 各種項目設定 */
		...
		/* ファイル名 */
		$file_name = "super_fire_racing_m.php";

		/* 商品名 */
		$set_name = "HKS SUPER FIRE RACINGプラグMシリーズ";

		/* DBパス */
		$set_db = "db/super_fire_racing_m.csv";
...
<?php
	...
	print "				<td>"; print_r($db_data[0][10]); print "</td>\n";
	print "				<td>"; print_r($db_data[0][11]); print "（税抜）</td>\n";
	print "			</tr>\n";
	/* 適合データの処理 */
	for($cnt = 1; $cnt < $i; $cnt++){
		...
		if($db_data[$cnt][0] == $maker){
			...
			print "				<td>"; print_r($db_data[$cnt][10]); print "</td>\n";
			/* 販売価格に3桁毎のカンマを追加処理 */
			$price1 = number_format($db_data[$cnt][11]);
			$price2 = number_format($db_data[$cnt][12]);
...
?>
		...
		<div class="footer_text_t">
			<div class="footer_text_c1">※：</div>
			<div class="footer_text_c2">本ページ掲載価格は1台分の合計商品価格となり、取付工賃等は含まれておりません。取付ご希望の場合は別途お問い合わせ下さい。</div>
		</div>
		<div class="footer_text_t">
			<div class="footer_text_c1">※：</div>
