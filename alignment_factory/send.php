<?php
	...
	/* ワゴン車輌の場合（全テスターで対応可能な為、規制解除中 */
	$sheet_check = 0;
	/* if($wagon_check == 1 and $lift == 1 and $change_lift != 0){$lift = $change_lift;} */
	/* elseif($wagon_check == 1 and $lift == 1 and $change_lift == 0){$sheet_check = 1;} */
...
<?php
	...
	file_put_contents("../work_sheet_factory/log/$work_file","$in_data"); /* 配列としてではなくカンマ・改行付きの文字列として保存（CSV形式なので問題無し） */
	/* 作業一覧ファイル */
	...
	/* 最終更新日時の更新 */
	file_put_contents("../work_sheet_factory/ymd.dat","$date");
?>
...
<?php
	...
	/* 件名 */
	$subject = mb_encode_mimeheader("【KTS FACTORY】アライメント予約");
	
	/* 宛先 */
	$to = $mail;
	
	/* headers */
	$headers = "From: alignment-factory@kts-web.com\n";
	/* $headers = "From: sugawara@kts-web.com\n"; */
	$headers .= "BCC: sugawara@kts-web.com,k.hanaki@kts-web.com,alignment-factory@kts-web.com\n";
	...
	$body .= "KTS FACTORY" . "\n";
	$body .= "TEL：048-285-8906" . "\n";

	/* Return-Path */
	$mparameter = "-f alignment-factory@kts-web.com";
	/* メール送信 */
