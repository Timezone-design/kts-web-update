<?php
	...
	/* 各種項目設定 */
		...
		/* ファイル名 */
		$file_name = "brembo_rotor_fbd.php";

		/* SETメニュー名 */
		$set_name = "brembo フローティングディスクブレーキ取付SETメニュー";

		/* DBパス */
		$set_db = "db/brembo/rotor_fbd.csv";

		/* Campスイッチ 0：通常　1：バナー1　2：バナー2　3：バナー3 */
		$camp_b_switch = 0;
...
?>
	<head>
		...
		<link rel="stylesheet" href="../set_php.css" type="text/css" />
...
<?php
	...
	if($camp_b_switch == 3){print "			<div class=camp_b><img src=../img/camp_b_3.gif></div>\n";}
	/* メーカーアイコン処理（PC） */
	$icon_pc_cnt = 1;
...
<?php
	if($car_name_status == 1){
		...
		print "				<td>"; print_r($db_data[0][8]); print "（税抜）</td>\n";
		print "			</tr>\n";
		/* 適合データの処理 */
		for($cnt = 1; $cnt < $i; $cnt++){
			...
			if($db_data[$cnt][0] == $maker){
				...
				print "				<td>"; print_r($db_data[$cnt][6]); print "</td>\n";
				print "				<td>"; print_r($db_data[$cnt][7]); print "</td>\n";
				/* 販売価格に3桁毎のカンマを追加処理 */
				$price1 = number_format($db_data[$cnt][8]);
				$price2 = number_format($db_data[$cnt][9]);
...
?>
		<div class="footer_text_t">
			...
			<div class="footer_text_c2">SET価格の金額をクリックすると商品単体で購入可能な弊社直販サイトへリンク致します。</div>
		</div>
		<div class="footer_text_t">
			<div class="footer_text_c1">※：</div>
			<div class="footer_text_c2">直販サイトから取付SETメニューのご注文も可能です。</div>
		</div>
		<div class="footer_text_t">
			<div class="footer_text_c1">※：</div>
		...
		<div class="footer_text_t">
			<div class="footer_text_c1">※：</div>
			<div class="footer_text_c2">輸入車についてはFACTORY店での作業は行っておりません。平和島店または一之江店での作業となります。</div>
		</div>
