<?php
	...
	/* 各種項目設定 */
		...
		/* ファイル名 */
		$file_name = "endless_pad_ssy.php";

		/* SETメニュー名 */
		$set_name = "ENDLESS ブレーキパッド SSY（Super Street Y-sports）取付SETメニュー";

		/* DBパス */
		$set_db = "db/endless/pad_ssy.csv";
...
<?php
	if($car_name_status == 1){
		...
		print "				<td>"; print_r($db_data[0][7]); print "（税抜）</td>\n";
		print "			</tr>\n";
		/* 適合データの処理 */
		for($cnt = 1; $cnt < $i; $cnt++){
			...
			if($db_data[$cnt][0] == $maker){
				...
				/* 販売価格に3桁毎のカンマを追加処理 */
				$price1 = number_format($db_data[$cnt][7]);
				$price2 = number_format($db_data[$cnt][8]);
