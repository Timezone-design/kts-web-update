<?php
	...
	/* 各種項目設定 */
		...
		/* ファイル名 */
		$file_name = "fgk_power_getter.php";

		/* SETメニュー名 */
		$set_name = "FUJITSUBO POWER GETTER取付SETメニュー";

		/* DBパス */
		$set_db = "db/fgk/power_getter.csv";
...
<?php
	...
	print "				<td>"; print_r($db_data[0][4]); print "（税抜）</td>\n";
	print "			</tr>\n";
	/* 適合データの処理 */
	for($cnt = 1; $cnt < $i; $cnt++){
		...
		if($db_data[$cnt][0] == $maker){
			...
			/* 販売価格に3桁毎のカンマを追加処理 */
			$price1 = number_format($db_data[$cnt][4]);
			$price2 = number_format($db_data[$cnt][5]);
