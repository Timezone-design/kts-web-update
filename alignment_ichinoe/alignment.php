<?php
	...
	function filedata(){
		...
		$file_name = "../work_sheet_ichinoe/log/$year$mon0$mday0.dat";
	...
	/* 最終更新日時DATファイル呼び出し */
	$fp_ymd = fopen("../work_sheet_ichinoe/ymd.dat","r");
	$ymd = fgets($fp_ymd);
	fclose($fp_ymd);
...
?>
	...
	<body bgcolor="#FFFFFF" text="#000000">
		<table width="800" border="0">
			...
			<tr><td><b><font size="3"><a href="http://www.kts-web.com/shop_menu/tenpo/ichinoe.html" target="_blank">KTS一之江店</a> TEL: 03-3674-2006</font></b></td></tr>
		</table>
		<table width="870">
			<tr>
				<td width="30"></td>
			...
			<tr>
				<td></td>
				<td>
					<div><font color="#FF0000"><b>KTS一之江店ではアライメントテスターを2基ご用意しており、同時に2台までアライメント作業が可能です。</b></font></div>
					<div><font color="#FF0000"><b>しかし、リフトの都合上、片方のラインではワゴン・ミニバン系車輌の作業が行えません。</b></font></div>
					<div><font color="#FF0000"><b>その為、同日時間帯にワゴン・ミニバン系車輌を同時に作業することができず、ご予約が重なった際には先着順とさせて頂き、後からのご予約をお断りさせて頂く場合がございます。ご迷惑をお掛け致しますが予めご了承下さい。</b></font></div>
				</td>
			</tr>
		</table>
		...
		<table width="800" border="0" bgcolor="#66CCFF">
			<tr>
				<td>　</td>
...
<?php
	for($i = $days_range; $i <= 6; $i++){
		...
		while($jikan <= 10){
			...
			/* 第2工場アライメントリフトの確認 */
			if($day_para[10][$check_time] != "open" and $day_para[10][$check_time] != ""){$lift2 = 0;}
			if($day_para[10][$check_time_h] != "open" and $day_para[10][$check_time_h] != ""){$lift2 = 0;}
			/* リフト空き枠の確認 */
			$lift = $lift1 + $lift2;
			/* 9時枠処理（基本時間外扱い）*/
			/* if($jikan == 1){print "<td><img src=https://www.kts-web.com/alignment_ichinoe/jikangai.gif></td>\n";} */
			if($jikan == 1){print "<!--<td><img src=https://www.kts-web.com/alignment_ichinoe/jikangai.gif></td>\n-->";}
			/* GW休業の処理2019 臨時処理 */
			/* elseif($mon == 5 and $mday == 3){print "<td><img src=https://www.kts-web.com/alignment_factory/jikangai.gif></td>\n";} */
